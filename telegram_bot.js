...
// b represents Size Bet (e.g. 10, 30, 90, 270, 810)
// n represents Number Bet per number. Total number bet (2 numbers) = 2 * n, which is 20% of b (10% each)
const LVL_BASE = [
    {b: 10, n: 1},  // L1: Size 10, Numbers: 1 + 1 (Total: 12)
    {b: 30, n: 3},  // L2: Size 30, Numbers: 3 + 3 (Total: 36)
    {b: 90, n: 9},  // L3: Size 90, Numbers: 9 + 9 (Total: 108)
    {b: 270, n: 27},// L4: Size 270, Numbers: 27 + 27 (Total: 324)
    {b: 810, n: 81} // L5: Size 810, Numbers: 81 + 81 (Total: 972)
];

const SCALE_BASE = 200; // Base wallet scale updated to 200
...
// 30-Box Neural Pattern-Matrix layout
let boxState = {
    group1: Array(10).fill(null), 
    group2: Array(10).fill(null), 
    group3: Array(10).fill(null)  
};

let autoPostTimeout = null;
let isPollingReconnecting = false; // Flag to prevent multiple reconnect loops on 409 error
...
let periodResults = [];      // timeline records (WIN, LOSS, JACKPOT)
...
// Force IPv4 using family: 4 within the underlying HTTP agent to bypass DNS AggregateError on Render/Node 17+
...
// Self-Healing Logic for 409 Conflict and other Polling Errors
bot.on('polling_error', (error) => {
    const errorMsg = error.message || String(error);
    console.error("⚠️ [Telegram Polling Error]:", errorMsg);

    if (errorMsg.includes('409 Conflict') && !isPollingReconnecting) {
        isPollingReconnecting = true;
        console.log("🔄 [Self-Healing]: 409 Conflict डिटेक्ट हुआ! पोलिंग को 15 सेकंड के लिए रोका जा रहा है ताकि पुराना इंस्टेंस बंद हो सके...");
        
        bot.stopPolling()
            .then(() => {
                setTimeout(() => {
                    console.log("🔄 [Self-Healing]: अब फ्रेश पोलिंग रीस्टार्ट की जा रही है...");
                    bot.startPolling({ restart: true })
                        .then(() => {
                            isPollingReconnecting = false;
                            console.log("🟢 [Self-Healing]: पोलिंग सफलतापूर्वक सिंक हो गई है।");
                        })
                        .catch(err => {
                            isPollingReconnecting = false;
                            console.error("❌ दोबारा पोलिंग शुरू करने में दिक्कत:", err.message);
                        });
                }, 15000); // Wait 15 seconds safely before reconnecting
            })
            .catch(err => {
                isPollingReconnecting = false;
                console.error("❌ पोलिंग रोकने में विफल:", err.message);
            });
    }
});
...
const server = http.createServer((req, res) => {
    ...
    res.end('Trades Star Bot running securely with Upgraded 3X Scale!\n');
});
...
const DEFAULT_SAFE_SLOTS = [
    { start: "11:00 AM", end: "01:00 PM", label: "UNDER 2 LEVEL" },
...
function extractSafeSlots(timelineRecords, maxAllowedLevel = 2, minDuration = 10) {
        ...
        if (cycle.maxLevel <= maxAllowedLevel) {
            ...
            if (currentSlotStart !== null && currentSlotEnd !== null) {
                ...
                if (duration >= minDuration) {
                    ...
                    const endAdjusted = minutesToTime12((endMin - 1 + 1440) % 1440);
                    safeSlots.push({
                        start: startAdjusted,
                        end: endAdjusted,
...
function getSafeTimeSlots() {
    ...
    const allowedLevel = 2; // Strict Level-2 Threshold
...
async function refreshDailyPinnedMessage() {
    try {
        ...
        const pinMsg = `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                       ...
                       `━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                       `यह कोई साधारण प्रेडिक्शन चैनल नहीं है। यह विंगो (Wingo 1M) गेम के इतिहास का सबसे उन्नत, वैज्ञानिक और 100% गणितीय AI सिग्नल्स (30-Box Neural Matrix) का घर है।\n\n` +
                       ...
                       `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                       `📊 <b>5-LEVEL 3X MARTINGALE PROGRESSIVE CHART</b>\n` +
                       `━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                       `📶 <b>L1:</b> ₹10 | ₹1 (on each predicted number)\n` +
                       `📶 <b>L2:</b> ₹30 | ₹3 (on each predicted number)\n` +
                       `📶 <b>L3:</b> ₹90 | ₹9 (on each predicted number)\n` +
                       `📶 <b>L4:</b> ₹270 | ₹27 (on each predicted number)\n` +
                       `📶 <b>L5:</b> ₹810 | ₹81 (on each predicted number)\n\n` +
                       `👉 इस वैज्ञानिक 3X प्रगतिशील सिस्टम से लॉस practically impossible है\n\n` +
                       `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                       `🔗 <b>IMPORTANT LINKS</b>\n` +
                       `━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                       `📱 <b>REGISTER LINKS:</b>\n` +
        ...
        // Added 1-second delay for Telegram backend to process the message before attempting to pin
        setTimeout(async () => {
            try {
                await bot.pinChatMessage(CHANNEL_CHAT_ID, sent.message_id, { disable_notification: true });
                pinnedMessageId = sent.message_id;
                console.log(`📌 Daily fresh message pinned securely on channel in IST. ID: ${pinnedMessageId}`);
            } catch (pinErr) {
                console.error("⚠️ Failed to PIN message. Make sure the bot is an ADMIN with 'Pin Messages' privileges:", pinErr.message);
            }
        }, 1000);
...
bot.onText(/\/start/, (msg) => {
    ...
    const startMsg = `🌟 <b>WELCOME TO TRADE'S STAR V20 PRO VIP BOT</b> 🌟\n\n` +
                     ...
                     `👉 /30box - 30 बॉक्स की लाइव कैलकुलेशन और Numbers देखें।\n` +
...
bot.onText(/\/prediction/, async (msg) => {
    ...
    if (session) {
        ...
        if (session.step === 'active') {
            ...
            if (session.type === 'low_balance') {
                ...
                if (!checkSafe.isSafe) {
                    ...
                    const unsafeMsg = `⚠️ <b>अभी आपका लॉस हो सकता है!</b>\n\n` +
                                      `चूंकि आपका बजट छोटा (₹200 से ₹1999) है, इसलिए सुरक्षित प्रेडिक्शन के लिए कृपया <b>${nextSlot.start} से ${nextSlot.end}</b> के बीच तैयार रहें। तब आपको सुरक्षित प्रेडिक्शन दिया जाएगा!`;
                    await bot.sendMessage(chatId, unsafeMsg, { parse_mode: 'HTML' }).catch(() => {});
                    return;
                }

                // Low budget mode with 3X progression
                const bBet = 10 * Math.pow(3, session.activeLevel - 1);
                ...
                const lowBalMsg = `🚨 <b>PREDICTION LIVE</b> 🚨\n` +
                                  ...
                                  `🪙 N•BET.  :- ₹0\n` +
                                  `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                                  `📊 LEVEL.  :- ${session.activeLevel} (3X PROGRESSIVE)\n` +
                                  `━━━━━━━━━━━━━━━━━━━━━━━`;
            ...
            // Scale calculations on standard base 200
            const scale = session.currentBalance / SCALE_BASE;
    ...
    if (!currentPrediction) {
        bot.sendMessage(chatId, `⚠️ लाइव API सिंकिंग की प्रतीक्षा करें... 5 संकेत में दोबारा प्रयास करें।`).catch(() => {});
        return;
    }
...
bot.onText(/\/editwallet(?:\s+)?(?:<)?(\d+)(?:>)?/i, (msg, match) => {
    ...
    if (!isNaN(amount) && amount >= 200) {
        ...
        if (amount < 2000) {
            ...
            bot.sendMessage(chatId, `💰 <b>Wallet Balance updated to (Low Budget Mode):</b> ₹${amount}\n\nअब प्रेडिक्शन केवल सुरक्षित स्लॉट्स में ₹10 -> ₹30 -> ₹90... प्रोग्रेसिव साइज पर मिलेंगे!`, { parse_mode: 'HTML' }).catch(() => {});
        } else {
        ...
        if (session.step === 'awaiting_balance') {
            ...
            if (balance < 2000) {
                ...
                const successMsg = `🚀 <b>लॉस कवर सेशन (LOW BUDGET MODE) शुरू हो चुका है!</b>\n\n` +
                                   ...
                                   `👉 आपको केवल <b>BIGGG/SMALL</b> प्रेडिक्शन मिलेंगे।\n` +
                                   `👉 प्रोग्रेसिव बेट: <b>₹10 -> ₹30 -> ₹90 -> ₹270 -> ₹810...</b>\n` +
...
bot.onText(/\/chart/, (msg) => {
    const chatId = msg.chat.id;
    const scale = walletBalance / SCALE_BASE; // Math scaled relative to base budget 200
    
    let chartMsg = `📊 <b>5-LEVEL SCALED 3X MARTINGALE CHART</b>\n`;
    ...
    LVL_BASE.forEach((lvl, i) => {
        ...
        const total = b + (n * 2) + totalLoss; // Bet size + Bet on 2 numbers + accumulated losses
        const bProf = Math.round(b * 1.96) - total;
        const nProf = Math.round(n * 9) - total; // 9x payout representation
    ...
    chartMsg += `--------------------------------------------------------\n`;
    chartMsg += `⚠️ <b>Note:</b> Bet (B) का मतलब Size (Big/Small) है और Bet (N) का मतलब Number है (दोनो नंबर पर Bet (N) सामान रूप से लगाया जायेगा)।`;
...
function generateBoxMessage() {
    if (!currentPrediction) {
        return `⚠️ अभी तक कोई लाइव डेटा विश्लेषित नहीं हुआ है।`;
    }
    
    const showVal = (val) => val !== null ? `[ ${val} ]` : `[ ? ]`;
    
    let text = `📊 <b>LIVE 30-BOX NEURAL MATRIX ANALYSIS</b> 📊\n\n` +
               `📅 <b>Next Period ID:</b> #${currentPrediction.issue.slice(-4)}\n\n` +
               `📦 <b>Group 1: Period-Step Scan (P-10 to P-100)</b>\n` +
               `  ` + boxState.group1.map(v => showVal(v)).join(' ') + `\n\n` +
               `📦 <b>Group 2: Jackpot Hit Match Scan (Last 10 Matches)</b>\n` +
               `  ` + boxState.group2.map(v => showVal(v)).join(' ') + `\n\n` +
               `📦 <b>Group 3: Double Sequence Pattern Scan (Last 10 Matches)</b>\n` +
               `  ` + boxState.group3.map(v => showVal(v)).join(' ') + `\n\n` +
               `🎯 <b>Computed Prediction Target:</b> <b>${getFancyType(currentPrediction.predType)}(${currentPrediction.nums.join(',')})</b>`;
    return text;
}

bot.onText(/\/(8box|30box)/, (msg) => {
    const chatId = msg.chat.id;
    const responseMsg = generateBoxMessage();
    bot.sendMessage(chatId, responseMsg, { parse_mode: 'HTML' }).catch(() => {});
});
...
bot.onText(/\/ai/, (msg) => {
    ...
    const opts = {
        reply_markup: {
            inline_keyboard: [
                [{ text: '📊 Q1. लाइव विन रेट (Win Rate)', callback_data: 'ai_q1' }],
                [{ text: '🔥 Q2. मैक्सिमम स्ट्रीक रिकॉर्ड (Max Streak)', callback_data: 'ai_q2' }],
                [{ text: '⏰ Q3. सबसे सुरक्षित ट्रेडिंग स्लॉट (Safe Slots)', callback_data: 'ai_q3' }],
                [{ text: '🧠 Q4. क्या तुम विंगो एक्सपर्ट हो? (AI Expert)', callback_data: 'ai_q4' }],
                [{ text: '👥 Q5. तुम्हारे पीछे की टीम कौन है? (Team)', callback_data: 'ai_q5' }],
                [{ text: '🛡️ Q6. छोटा बजट (< ₹2000) का प्लान (Small Budget)', callback_data: 'ai_q6' }],
                [{ text: '⚙️ Q7. 30-Box मॉडल कैसे काम करता है? (How it works)', callback_data: 'ai_q7' }],
                [{ text: '💰 Q8. सेफ ट्रेडिंग के लिए बेस्ट बजट (Best Budget)', callback_data: 'ai_q8' }],
                [{ text: '🔄 Q9. लेवल 5 लॉस के बाद रिकवरी प्लान (Recovery Plan)', callback_data: 'ai_q9' }],
                [{ text: '🎲 Q10. 20% नंबर हेजिंग का क्या फायदा है? (20% Hedging)', callback_data: 'ai_q10' }]
            ]
        }
    };
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        // Stop the loading clock icon immediately in Telegram
        await bot.answerCallbackQuery(callbackQuery.id).catch(() => {});

        if (data === 'get_next_pred_personal') {
            const session = userSessions[userId];
            if (!session || session.step !== 'active') {
                await bot.sendMessage(chatId, "❌ कोई सक्रिय लॉस कवर सेशन नहीं मिला!").catch(() => {});
                return;
            }
            if (!currentPrediction) {
                await bot.sendMessage(chatId, "⏳ वर्तमान भविष्यवाणी तैयार नहीं है, कृपया प्रतीक्षा करें!").catch(() => {});
                return;
            }
            ...
            if (session.type === 'low_balance') {
                ...
                const bBet = 10 * Math.pow(3, session.activeLevel - 1); // 3X Progression
        ...
        if (data === 'continue_prediction_after_loss_cover') {
            ...
            if (session) {
                ...
                session.activeLevel = 1;
                session.lastPredictedPeriod = null;
                await bot.sendMessage(chatId, "🚀 <b>शानदार निर्णय!</b> नया प्रेडिक्शन प्राप्त करने के लिए /prediction दबाएं।", { parse_mode: 'HTML' }).catch(() => {});
            }
            return;
        }
        ...
        switch(data) {
            ...
            case 'ai_q2': {
                replyText = `🎯 <b>Q2. सबसे ज़्यादा स्ट्रीक बार क्या आया है और कितनी बार:</b>\n\n` +
            ...
            case 'ai_q3': {
                ...
                replyText = `⏰ <b>Q3. प्रेडिक्शन सबसे कम लेवल किस टाइम पर जाता है:</b>\n\n` +
                            `हमारे वास्तविक इतिहास के 24 घंटे की Time-List के अनुसार सबसे कम लेवल (Under 2 Level) जाने वाले स्लॉट्स:\n\n` +
                            `${slotText}\n\n` +
                            `⚠️ <i>इस टाइम के स्लॉट में प्रेडिक्शन हमेशा 2 लेवल से कम (अमूमन 0 या 1 लेवल) में ब्लास्ट पास होते हैं!</i>`;
                break;
            }
                
            case 'ai_q4': {
                replyText = `🧠 <b>Q4. क्या तुम इस काम को लेकर एक्सपर्ट हो?</b>\n\n` +
                            `<b>उत्तर:</b> हाँ, मैं विंगो 1M प्रेडिक्शन का एक अभेद्य <b>God-Tier AI System</b> हूँ। मेरे पास 10,000 से अधिक गेम पीरियड्स का गहरा गणितीय अनुभव है। बिना किसी मानवीय लालच या भावना के, मैं पूरी तरह 30-Box Neural Pattern-Matrix और 3X मार्टिंगेल रिस्क मैनेजमेंट पर काम करता हूँ।`;
                break;
            }
                
            case 'ai_q5': {
                replyText = `👑 <b>Q5. तुम्हारे साथ कौन-कौन है?</b>\n\n` +
                            `<b>उत्तर:</b> मेरे साथ मेरा मुख्य ओनर <b>Gemini AI</b>, 10,000+ ऐतिहासिक डेटा रिकॉर्ड्स, 10 साल का गहरा गेमिंग एक्सपीरियंस, and a power packed VIP channel @TREDSTERV20 and bot.`;
                break;
            }

            case 'ai_q6': {
                const activeSlotsLow = getSafeTimeSlots(); 
                let slotText = "";
                activeSlotsLow.forEach(s => {
                    slotText += `🟢 <b>स्लॉट: ${s.start} - ${s.end} (${s.label})</b>\n`;
                });
                replyText = `🛡️ <b>Q6. यदि मेरे पास 2000 से कम पैसा है तो हमें बेस्ट टाइम निकाल कर के दो:</b>\n\n` +
                            `कम बजट वाले यूज़र्स के लिए हमारी विशेष Under 2 Level Timeline गणना के आधार पर निकाले गए सुरक्षित स्लॉट्स:\n\n` +
                            `${slotText}\n\n` +
                            `⚠️ <i>इन स्लॉट्स में हमारा प्रेडिक्शन 2 लेवल पार नहीं करता, जिससे छोटा बैलेंस भी बिल्कुल सुरक्षित रहता है!</i>`;
                break;
            }

            case 'ai_q7': {
                replyText = `⚙️ <b>Q7. 30-Box Neural Matrix मॉडल कैसे काम करता है?</b>\n\n` +
                            `<b>उत्तर:</b> यह मेरा सबसे एडवांस सीक्रेट वेपन है! यह 3 अलग-अलग 10-बॉक्स ग्रुप्स में काम करता है:\n\n` +
                            `1. <b>Group 1 (Box 1-10):</b> यह पिछले $P-10$ से $P-100$ पीरियड्स के स्टेप ट्रेंड को स्कैन करता है।\n` +
                            `2. <b>Group 2 (Box 11-20):</b> यह पिछले जैकपॉट विनर नंबर ($N_0$) के पिछले 10 इतिहास को ट्रैक करके उसके ठीक बाद आने वाले नंबरों को जमा करता है।\n` +
                            `3. <b>Group 3 (Box 21-30):</b> यह आखिरी 2-डिजिट सीक्वेंस ($N_1 \\rightarrow N_0$) पैटर्न को मैच करता है।\n\n` +
                            `इन सभी 30 बॉक्सों की मेजॉरिटी वोटिंग के आधार पर ही आखिरी साइज (BIG/SMALL) और बेस्ट 2 हेजिंग नंबरों की सटीक गणना की जाती है!`;
                break;
            }

            case 'ai_q8': {
                replyText = `💰 <b>Q8. सुरक्षित ट्रेडिंग के लिए बेस्ट बजट क्या होना चाहिए?</b>\n\n` +
                            `<b>उत्तर:</b> बोट 3X प्रोग्रेसिव मनी मैनेजमेंट पर काम करता है।\n` +
                            `• <b>बेस्ट बजट:</b> ₹1000 से ₹4000 के बीच सबसे परफेक्ट माना जाता है।\n` +
                            `• ₹2000+ बैलेंस रखने पर आप हमारे standard <b>₹10 -> ₹30 -> ₹90 -> ₹270 -> ₹810</b> के मार्टिंगेल सिस्टम को आसानी से मेंटेन रख सकते हैं। इससे आपका रिस्क 0% हो जाता है और 10% डेली टारगेट बहुत आसानी से हासिल हो जाता है!`;
                break;
            }

            case 'ai_q9': {
                replyText = `🔄 <b>Q9. अगर लगातार 5 लेवल लॉस हो जाये तो क्या करें?</b>\n\n` +
                            `<b>उत्तर:</b> शांत रहें और पैनिक बिल्कुल न करें! गणितीय नियमों के अनुसार 30-Box Neural Matrix लगातार 5 लेवल लॉस होने की संभावना 0.1% से भी कम रखता है।\n\n` +
                            `अगर कभी रेयर केस में ऐसा होता है, तो तुरंत प्रेडिक्शन बंद करें और 2 घंटे के कूलडाउन में जाएं। उसके बाद /losscover कमांड दबाएं, बोट आपके शेष बैलेंस के अनुसार प्रोग्रेसिव चार्ट को री-ट्यून कर देगा और आपका लॉस धीरे-धीरे पूरी तरह कवर हो जायेगा!`;
                break;
            }

            case 'ai_q10': {
                replyText = `🎲 <b>Q10. 20% नंबर हेजिंग का क्या फायदा है?</b>\n\n` +
                            `<b>उत्तर:</b> सामान्य प्रेडिक्टर सिर्फ BIG या SMALL बताते हैं, जिससे प्रॉफिट 1.96 गुना होता है। लेकिन हमारा बोट:\n` +
                            `• आपके दांव का <b>80% हिस्सा</b> मेन साइज (BIG/SMALL) पर लगाता है।\n` +
                            `• बाकी का <b>20% हिस्सा (10% + 10%)</b> हमारे चुने गए 2 हेजिंग नंबरों पर लगाता है।\n\n` +
                            `इससे फायदा यह होता है कि जब जैकपॉट (नंबर हिट) आता है, तो आपका सीधा <b>9X (नौ गुना)</b> का जैकपॉट पास होता है, जिससे पिछले सारे लेवल का लॉस सिर्फ एक ही पीरियड में बड़ी जीत के साथ रिकवर हो जाता है!`;
                break;
            }
        }

        if (replyText) {
            await bot.sendMessage(chatId, replyText, { parse_mode: 'HTML', disable_web_page_preview: true }).catch(err => {
                console.error("Failed to deliver AI answer message:", err.message);
            });
        }
    } catch (err) {
        console.error("Error inside callback_query handler:", err.message || err);
    }
});
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        for (let userId in userSessions) {
            ...
            if (session.step === 'active' && session.lastPredictedPeriod === latestId) {
                ...
                if (session.type === 'low_balance') {
                    // low balance mode 3X progression
                    const sBetB = 10 * Math.pow(3, session.activeLevel - 1);
                    ...
                    const sScale = session.currentBalance / SCALE_BASE;
                ...
                if (netProfit >= session.lossAmount) {
                    ...
                    session.cooldownUntil = getISTTime().getTime() + 2 * 60 * 60 * 1000; 
    ...
    // ==========================================
    // 🧠 30-BOX NEURAL MATRIX ALGORITHM GENERATION
    // ==========================================

    // 1️⃣ Group 1 (Boxes 1 - 10): P-10 to P-100 scan step loop
    boxState.group1 = [];
    for (let step = 10n; step <= 100n; step += 10n) {
        let targetId = (P - step).toString();
        let found = history.find(h => h.issue === targetId);
        boxState.group1.push(found ? found.number : null);
    }

    // 2️⃣ Group 2 (Boxes 11 - 20): Latest Single Jackpot Occurrence above values
    const N0 = latestOutcome.number;
    boxState.group2 = [];
    let occurrenceCount = 0;
    for (let i = 1; i < history.length; i++) {
        if (history[i].number === N0) {
            if (history[i - 1] !== undefined) {
                boxState.group2.push(history[i - 1].number);
                occurrenceCount++;
                if (occurrenceCount >= 10) break;
            }
        }
    }
    while (boxState.group2.length < 10) {
        boxState.group2.push(null);
    }

    // 3️⃣ Group 3 (Boxes 21 - 30): Double Sequence [N1 -> N0] Pattern Scan
    boxState.group3 = [];
    if (history.length >= 2) {
        const N0_val = history[0].number;
        const N1_val = history[1].number;
        let patternCount = 0;
        
        for (let i = 1; i < history.length - 1; i++) {
            if (history[i].number === N0_val && history[i + 1].number === N1_val) {
                if (history[i - 1] !== undefined) {
                    boxState.group3.push(history[i - 1].number);
                    patternCount++;
    ...
    while (boxState.group3.length < 10) {
        boxState.group3.push(null);
    }
    ...
    if (solvedInputs.length > 0) {
        ...
        if (bigCount >= smallCount) {
            finalSelection = "BIG";
            finalNums = getOppositeHedgingNums([5, 6, 7, 8, 9], history);
        } else {
            finalSelection = "SMALL";
            finalNums = getOppositeHedgingNums([0, 1, 2, 3, 4], history);
        }
    }
    ...
    if (count0 > 1 && !finalNums.includes(0) && (finalSelection === "SMALL")) {
        finalNums[1] = 0;
    }
    if (count5 > 1 && !finalNums.includes(5) && (finalSelection === "BIG")) {
        finalNums[1] = 5;
    }
...
async function sendStickerSafe(chatId, stickerId, fallbackEmoji) {
    try {
        if (!stickerId || typeof stickerId !== 'string') {
            throw new Error("Sticker ID is undefined or not a valid string.");
        }
        
        const cleanStickerId = stickerId.trim();
        await bot.sendSticker(chatId, cleanStickerId);
        console.log(`✅ Sticker sent successfully to ${chatId}: ${cleanStickerId.slice(0, 15)}...`);
    } catch(err) {
        console.error(`⚠️ Sticker transmission failed for chatId: ${chatId}. Reason: ${err.message}. Resorting to fallback emoji.`);
        try {
            await bot.sendMessage(chatId, fallbackEmoji);
        } catch (sendErr) {
            console.error("❌ Fallback emoji delivery failed:", sendErr.message);
        }
    }
}

setInterval(monitorLoop, 5000);
console.log("Monitoring Wingo 1M active loop on Upgraded 3X progressive engine...");
