/**
 * 🌟 TRADE'S STAR V20 PRO VIP - TELEGRAM AUTO BOT (WEBHOOK EDITION) 🌟
 * 
 * Yeh bot 1-Minute Wingo data monitor karega aur channel par automation se predictions post karega.
 * Features:
 *  - Render Server Support (Webhooks Auto-Binding to bypass 409 Conflict).
...
let walletBalance = 4000; // Default wallet balance
let lastUserRequest = {};

// --- DUAL MODE WEBHOOK AND POLLING AUTO-NEGOTIATOR ---
const PORT = process.env.PORT || 3000;
const RENDER_URL = process.env.RENDER_EXTERNAL_URL; // Autodetected by Render

let bot;
if (RENDER_URL) {
    bot = new TelegramBot(TELEGRAM_BOT_TOKEN, { webHook: { port: PORT } });
    bot.deleteWebHook({ drop_pending_updates: true }).then(() => {
        bot.setWebHook(`${RENDER_URL}/bot${TELEGRAM_BOT_TOKEN}`, { drop_pending_updates: true });
        console.log(`🤖 Render Webhook activated on Port: ${PORT} at ${RENDER_URL}`);
    });
} else {
...
bot.onText(/\/prediction/, (msg) => {
    ...
    const manualMsg = `🚨 *MANUAL PREDICTION REQUESTED* 🚨\n\n` +
...
async function handleNewOutcome() {
    ...
    const nextIdLast3Str = nextId.slice(-3); // e.g. "574"
    let b4 = null;
    
    // Check if any period in history already has the same last 3 digits
    ...
    if (inputs.length > 0) {
        ...
        // Resolve priority (Size takes precedence on tie)
        ...
        if (bigCount >= smallCount) {
            ...
            finalNums = (redCount >= greenCount) ? [6, 8] : [5, 7, 9]; 
        }
    }

    // Home 4-number Jackpot Compiler (Box 5 1-digit rule)
