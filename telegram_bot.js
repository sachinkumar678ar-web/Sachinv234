 ...
 *  - FIXED: Red/Green Prediction logic override bug resolved.
 */
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        if (currentPrediction.predType === "BIG" || currentPrediction.predType === "SMALL") {
            matchesSelection = (currentPrediction.predType === actualSize);
        } else {
            matchesSelection = (currentPrediction.predType === actualCol || actualCol === "VIOLET");
        }
        ...
        // Prepare the display result structure
        let resultDisplay = `${actualSize}(${actualNum})`;
        if (currentPrediction.predType === "RED" || currentPrediction.predType === "GREEN") {
            resultDisplay = `${actualCol}(${actualNum})`;
        }
        ...
        // Accurate Winner Sticker dispatching
        if (matchesNumber) {
            // Target Number hit -> JACKPOT WIN
            stats.jackpots++;
            stats.wins++;
            botLevel = 1;
            await sendStickerSafe(STICKER_JACKPOT, "🎇");
        } else if (matchesSelection) {
            // Dominant parameter hit -> WIN
            stats.wins++;
            botLevel = 1;
    ...
    if (inputs.length > 0) {
        ...
        let maxSizeVal = Math.max(bigCount, smallCount);
        let maxColorVal = Math.max(redCount, greenCount);

        let dominantSize = bigCount >= smallCount ? "BIG" : "SMALL";
        let dominantColor = redCount >= greenCount ? "RED" : "GREEN";

        // Determine whether size or color prediction dominates the current pattern
        if (maxColorVal > maxSizeVal) {
            // COLOR PREDICTION DOMINATES!
            finalSelection = dominantColor;

            if (dominantColor === "RED") {
                // RED dominant: hedge numbers based on dominant size
                finalNums = (dominantSize === "BIG") ? [2, 4] : [6, 8];
            } else {
                // GREEN dominant: hedge numbers based on dominant size
                finalNums = (dominantSize === "BIG") ? [1, 3] : [5, 7];
            }
        } else {
            // SIZE PREDICTION DOMINATES (OR EQUAL VALUE)!
            finalSelection = dominantSize;

            if (dominantSize === "BIG") {
                // BIG dominant: hedge numbers based on dominant color
                finalNums = (dominantColor === "RED") ? [2, 4] : [1, 3];
            } else {
                // SMALL dominant: hedge numbers based on dominant color
