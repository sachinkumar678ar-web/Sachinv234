...
async function refreshDailyPinnedMessage() {
    try {
        ...
        const pinMsg = `━━━━━━━━━━━━━━━━━━━━━━━\n` +
                       ...
                       `⚠️ **नियम:**\n` +
                       `कम से कम ₹200 बैलेंस होना जरूरी है\n\n` +
...
// Command: /prediction (Handles active session constraints, low-balance rules, and cooldown limits)
        ...
        if (session.step === 'active') {
            ...
            if (session.type === 'low_balance') {
                ...
                if (!checkSafe.isSafe) {
                    ...
                    await bot.sendMessage(chatId, unsafeMsg, { parse_mode: 'HTML' });
                    return;
                }
                ...
                const sentMsg = await bot.sendMessage(chatId, lowBalMsg, { parse_mode: 'HTML' });
...
bot.onText(/\/losscover/, async (msg) => {
    ...
    userSessions[userId] = {
        ...
        lastMessageId: null,
        type: 'standard' // 'standard' or 'low_balance'
    };
    ...
    if (session) {
        ...
        if (session.step === 'awaiting_balance') {
            ...
            if (isNaN(balance) || balance < 200) {
                await bot.sendMessage(chatId, "⚠️ <b>डिपॉजिट आवश्यक!</b> कृपया कम से कम <b>₹200</b> या इससे ज़्यादा का वॉलेट बैलेंस लिखें, क्योंकि इतने कम अमाउंट में लेवल मैनेज नहीं हो पाता।", { parse_mode: 'HTML' });
                return;
            }
            ...
            if (balance < 2000) {
                ...
                session.targetBalance = Math.round(balance + session.lossAmount); // Ultimate target is full loss cover
                session.step = 'active';
                session.activeLevel = 1;
                session.lastPredictedPeriod = null;
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    // 1. Next prediction handlers for personal sessions
    if (data === 'get_next_pred_personal') {
        ...
        if (session.type === 'low_balance') {
            ...
            if (!checkSafe.isSafe) {
                ...
                await bot.sendMessage(message.chat.id, unsafeMsg, { parse_mode: 'HTML' });
            ...
            const sentMsg = await bot.sendMessage(message.chat.id, lowBalMsg, { parse_mode: 'HTML' });
            session.lastMessageId = sentMsg.message_id;
            session.lastPredictedPeriod = currentPrediction.issue;
        } else {
            const scale = session.currentBalance / 3963;
    ...
    // Standard Personal next prediction button handler
    ...
    // Continue prediction handler after loss cover options choice
    if (data === 'continue_prediction_after_loss_cover') {
        ...
        if (session) {
            ...
            session.activeLevel = 1;
            session.lastPredictedPeriod = null;
            await bot.sendMessage(message.chat.id, "🚀 <b>शानदार निर्णय!</b> नया प्रेडिक्शन प्राप्त करने के लिए /prediction दबाएं।", { parse_mode: 'HTML' });
        }
        await bot.answerCallbackQuery(callbackQuery.id);
        return;
    }
        ...
        for (let userId in userSessions) {
            ...
            if (session.step === 'active' && session.lastPredictedPeriod === latestId) {
                ...
                if (netProfit >= session.lossAmount) {
                    ...
                    await bot.sendMessage(session.chatId, victoryMsg, { parse_mode: 'HTML', reply_markup: contOpts.reply_markup });
