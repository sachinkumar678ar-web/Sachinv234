...
const STICKER_WIN = 'CAACAgUAAxkBAAERUYpqHx2XZYHxYZSeDPJpA4V5fTkl_wACwRMAAsK2AAFUARL8QV6L3bI7BA'; 
const STICKER_LOSS = 'CAACAgEAAxkBAAERUY5qHx3Qhp14Lisxy8YJecLn4b-2SgACxgEAAiLw6Ee4H3D0MJr_azsE';
const STICKER_JACKPOT = 'CAACAgUAAxkBAAERUYxqHx3MgkHhV9GF3z0LDnpeURF0ZQACuBEAAi-IuFXIfWQ-6-g1TDsE';
...
// Helper: Parse time string "HH:MM AM/PM" to total minutes of the day
...
// Helper: Extract Safe Time Slots from any Timeline records (strictly underLevel maxLvl)
function extractSafeSlots(timelineRecords, maxAllowedLevel) {
...
function getSafeTimeSlots(useTimeline2 = false) {
    ...
    const under1 = extractSafeSlots(last24h, 1);
    const under2 = extractSafeSlots(last24h, 2);
...
// Helper: Post Daily Pinned Message at 12:00 AM IST
...
// Format Prediction Type for Fancy Layout (BIGGG, SMALL, REDDD, GREEN)
function getFancyType(type) {
    if (type === "BIG") return "BIGGG";
    if (type === "RED") return "REDDD";
    return type; // SMALL and GREEN remain same
}
    ...
    if (session) {
        ...
        if (session.step === 'active') {
            ...
            if (session.type === 'low_balance') {
                const checkSafe = isTimeInSafeSlot(true); // check timeline2 slots
                if (!checkSafe.isSafe) {
                    const nextSlot = checkSafe.slots[0] || DEFAULT_SAFE_SLOTS[0];
                    const unsafeMsg = `⚠️ <b>अभी आपका लॉस हो सकता है!</b>\n\n` +
                                      `चूंकि आपका बजट छोटा (₹200 से कम) है, इसलिए सुरक्षित प्रेडिक्शन के लिए कृपया <b>${nextSlot.start} से ${nextSlot.end}</b> के बीच तैयार रहें। तब आपको सुरक्षित प्रेडिक्शन दिया जाएगा!`;
...
// Command: /editwallet (Allows modifying the balance during session)
...
// Command: /ai - Interactive Inline Keyboard (6 Main Questions in Hindi)
bot.onText(/\/ai/, (msg) => {
    const chatId = msg.chat.id;
    const opts = {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: '❓ Q6. कम बजट सुरक्षित स्लॉट (नो-नंबर मॉडल)', callback_data: 'ai_q6' }]
            ]
        }
    };
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    try {
        ...
        switch(data) {
            ...
            case 'ai_q6': {
                const activeSlotsLow = getSafeTimeSlots(true); // low balance timeline2
                let slotText = "";
                activeSlotsLow.forEach(s => {
                    slotText += `🟢 <b>स्लॉट: ${s.start} - ${s.end} (${s.label})</b>\n`;
                });
                replyText = `🛡️ <b>Q6. कम बजट सुरक्षित स्लॉट (नो-नंबर मॉडल):</b>\n\n` +
                            `कम बजट (₹200 से कम) वाले यूज़र्स के लिए हमारी विशेष <b>Timeline 2</b> गणना के आधार पर निकाले गए सुरक्षित स्लॉट्स:\n\n` +
                            `${slotText}\n\n` +
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        // Resolve global low budget simulated level tracker for timeline2
        ...
        // Save entry for timeline2 (Low Budget model: jackpots treated as wins if size matched, else losses)
