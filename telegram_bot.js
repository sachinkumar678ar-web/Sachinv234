...
let history = [];
let botLevel = 1;
let lowBudgetLevelSim = 1; 
let walletBalance = 4000; 
let currentPrediction = null; 
let lastUserRequest = {};

// Upgraded state representing 3 collections of 10 boxes each (30-box model)
let boxState = {
    group1: Array(10).fill(null), // P-10 to P-100 values
    group2: Array(10).fill(null), // 10 Jackpot-matched previous elements
    group3: Array(10).fill(null)  // 10 Double-sequence matched previous elements
};
...
let periodResultsLow = [];   // timeline2 (WIN, LOSS - Jackpots treated as Loss)
...
// Advanced Safe Slot Generator based on User's Mathematical Description
...
function getSafeTimeSlots(useTimeline2 = false) {
    ...
    const allowedLevel = useTimeline2 ? 2 : 3; 
...
bot.onText(/\/start/, (msg) => {
    ...
    const startMsg = `🌟 <b>WELCOME TO TRADE'S STAR V20 PRO VIP BOT</b> 🌟\n\n` +
                     ...
                     `👉 /30box - 30 बॉक्स की लाइव कैलकुलेशन और Numbers देखें (वैकल्पिक कमांड: /8box).\n` +
    ...
    if (session) {
        ...
        if (session.step === 'active') {
            ...
            if (session.type === 'low_balance') {
                const checkSafe = isTimeInSafeSlot(true); 
                if (!checkSafe.isSafe) {
...
// Upgraded display to visualize the 30-Box neural matrix layout 
    ...
    const opts = {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: '❓ Q6. यदि मेरे पास 2000 से काम पैसा है तो हमे बेस्ट टाइम निकल कर के दो', callback_data: 'ai_q6' }]
            ]
        }
    };
        ...
        switch(data) {
            ...
            case 'ai_q4': {
                replyText = `🧠 <b>Q4. क्या तुम इस काम को लेकर एक्सपर्ट हो?</b>\n\n` +
                            `<b>उत्तर:</b> हाँ, मैं विंगो 1M प्रेडिक्शन का एक अभेद्य <b>God-Tier ai System</b> हूँ। मेरे पास 10,000 से अधिक गेम पीरियड्स का गहरा गणितीय अनुभव है। बिना किसी मानवीय लालच या भावना के, मैं पूरी तरह 30-Box Neural Pattern-Matrix और रिस्क मैनेजमेंट पर काम करता हूँ।`;
                break;
            }
            ...
            case 'ai_q6': {
                const activeSlotsLow = getSafeTimeSlots(true); 
                let slotText = "";
...
async function handleNewOutcome() {
    ...
    // Scan chronological backwards (forward indexing in history)
    for (let i = 1; i < history.length; i++) {
        if (history[i].number === N0) {
            // Find the number directly above it (chronologically newer -> i-1)
    ...
    // Pad array if we don't have enough history
    ...
    if (history.length >= 2) {
        ...
        // Scan history starting from index 1. 
        // We look for sequence pattern: history[i] is N0_val and older adjacent history[i+1] is N1_val
        for (let i = 1; i < history.length - 1; i++) {
            if (history[i].number === N0_val && history[i + 1].number === N1_val) {
                // Pick the element immediately chronologically newer/above it -> i-1
                if (history[i - 1] !== undefined) {
    ...
    // Resolve non-null entries from all 30 boxes
    let solvedInputs = [
        ...boxState.group1,
        ...boxState.group2,
        ...boxState.group3
    ...
    let finalNums = [5, 7]; // Fallback Big numbers
    ...
    if (solvedInputs.length > 0) {
        ...
        if (bigCount >= smallCount) {
            finalSelection = "BIG";
            // Ensure Big numbers correspond with BIG size
            finalNums = getOppositeHedgingNums([5, 6, 7, 8, 9], history);
        } else {
            finalSelection = "SMALL";
            // Ensure Small numbers correspond with SMALL size
    ...
    // Safeguard edge violet numbers (0/5) logic
