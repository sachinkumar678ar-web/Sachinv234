/**
 * 🌟 TRADE'S STAR V20 PRO VIP - TELEGRAM AUTO BOT 🌟
 * 
 * Configured Commands:
 ...
 *  - /ai : Interactive inline command center answering Q1-Q10 in Hindi.
...
function extractSafeSlots(timelineRecords, maxAllowedLevel, minDuration) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (sorted[i].status === "win") {
            ...
            // Under low balance timeline jackpot = loss, but standard timeline allows jackpot
            cycles.push({
                records: currentCycle,
                maxLevel: sorted[i].level,
                startTime: currentCycle[0].predTime || currentCycle[0].time,
        ...
        if (cycle.maxLevel <= maxAllowedLevel) {
            ...
            if (currentSlotStart !== null && currentSlotEnd !== null) {
                ...
                if (duration >= minDuration) {
                    // Reduce 1 minute from start and 2 minutes from end per rules
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        // Populate Timeline low timeline low results
    ...
    // Generate Prediction for NEXT upcoming period
    ...
    // Keep hedging strictly limited to exactly 2 numbers per rules
    finalNums = finalNums.slice(0, 2);

    // Keep hedging 0 & 5 frequently occurring
...
async function sendStickerSafe(chatId, stickerId, fallbackEmoji) {
    try {
        ...
        // Trim any invisible whitespaces from the sticker ID to bypass API rejects
