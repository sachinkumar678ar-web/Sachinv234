...
const CHANNEL_LINK = 'https://t.me/TREDSTERV20'; // ⚠️ यहाँ अपने चैनल का लिंक डालें

// 🟢 STICKERS CONFIGURATION (User Provided)
let STICKERS = {
    win: 'CAACAgUAAxkBAAEGxbhqpFBAYoBBfkci1nqKQMykp2clWQACbxQAAvvweFY9gMKaaaZ_Hz0E',
    loss: 'CAACAgUAAxkBAAERUmBqH_1UVPN7iQ-LBTKUs-NffGXV4AACkBEAAi4AATBXrdSO4Z9ezAM7BA', // Crying sticker
    jackpots: [
...
let history = []; 
let botLevel = 1; // अब अनलिमिटेड है
let walletBalance = 4000; 
let currentPrediction = null; 
let autoPostTimeout = null;
let isPollingReconnecting = false; 
let isBotActive = true; 
let skipMode = true; // Mode 1: Skip Mode ON (ख़राब ट्रेंड में स्किप करेगा)
...
// 📊 Stats Object
let stats = {
    total: 0, wins: 0, losses: 0, jackpots: 0, skips: 0, maxLevelReached: 1, 
    hourlyData: {}, pctTracker: {} // Admin के लिए परसेंटेज ट्रैकर
};

// 🧑‍💻 Loss Recovery Users (Private DM)
let recoveryUsers = {}; 
...
http.createServer((req, res) => { res.writeHead(200); res.end('Master AI V21 Active!\n'); }).listen(process.env.PORT || 3000);
...
// 6-LEVEL FUND MATH (Recovery & Channel)
...
// 🧠 AI SCANNER LOGIC
// ==========================================
function evaluatePattern(seq6, seq5, seq4, c1, c2) {
    if (seq5 === c1+c1+c2+c1+c1) return { pred: c2, chance: getRandomNumber(90, 95) };
    if (seq5 === c2+c2+c1+c2+c2) return { pred: c1, chance: getRandomNumber(90, 95) };
    if (seq6 === c2+c2+c2+c1+c1+c2) return { pred: c2, chance: getRandomNumber(92, 96) };
    if (seq4 === c1+c1+c1+c1) return { pred: c1, chance: getRandomNumber(85, 90) };
    if (seq4 === c2+c2+c2+c2) return { pred: c2, chance: getRandomNumber(85, 90) };
    if (seq4 === c1+c2+c1+c2) return { pred: c1, chance: getRandomNumber(80, 85) };
    if (seq4 === c1+c1+c2+c2) return { pred: c1, chance: getRandomNumber(75, 82) };
...
function generatePrediction(nextId) {
    ...
    let c0 = history[0].number % 2 === 0 ? "R" : "G"; let c1 = history[1].number % 2 === 0 ? "R" : "G";
    let c2 = history[2].number % 2 === 0 ? "R" : "G"; let c3 = history[3].number % 2 === 0 ? "R" : "G";
    let c4 = history[4].number % 2 === 0 ? "R" : "G"; let c5 = history[5].number % 2 === 0 ? "R" : "G";
    ...
    // 🔴 15% RULE: कलर तभी दिखाएगा जब कलर का चांस साइज से 15% ज़्यादा हो
    ...
    // 🔴 HIGH LEVEL RISK WARNING (अगर लेवल 6 या ज़्यादा है तो % गिरा दो)
    if (botLevel >= 5 && finalSelection !== "WAIT") {
        winChance = getRandomNumber(45, 55); 
    }

    // 🔴 SKIP MODE LOGIC
    if (finalSelection === "WAIT") {
        if (skipMode) {
            skipReason = "Market Unstable (Skip Mode ON)";
        } else {
            // अगर स्किप बंद है, तो कुछ भी प्रेडिक्शन दो कम % के साथ
            finalSelection = s0 === "B" ? "BIG" : "SMALL"; 
            winChance = getRandomNumber(50, 55); 
        }
    }
...
async function handleNewOutcome() {
    ...
    if (currentPrediction && currentPrediction.issue === latestId) {
        ...
        if (currentPrediction.predType === "WAIT") {
            ...
            // Admin Pct Tracker
            let range = (Math.floor(currentPrediction.chance / 10) * 10).toString(); // e.g., 80, 70, 50
            ...
            if (isWin || isJackpot) {
                ...
                if (isJackpot) {
                    ...
                    bot.sendSticker(CHANNEL_CHAT_ID, STICKERS.jackpots[Math.floor(Math.random() * STICKERS.jackpots.length)]).catch(()=>{});
                ...
                botLevel = 1; // Win होने पर ही 1 होगा (लॉस में अनलिमिटेड)
            } else {
                stats.losses++; consecutiveWins = 0; consecutiveJackpots = 0; 
                botLevel++; // अनलिमिटेड लेवल बढ़ता रहेगा
                bot.sendSticker(CHANNEL_CHAT_ID, STICKERS.loss).catch(()=>{}); // Crying Sticker
            }
...
// ==========================================
// ⏰ TIME BASED TRIGGERS (Morning, Night, Pin)
// ==========================================
setInterval(() => {
...
// ==========================================
// 💬 PRIVATE BOT - LOSS RECOVERY SYSTEM
// ==========================================
bot.on('message', (msg) => {
    const chatId = msg.chat.id, text = msg.text || "";
    if (msg.chat.type !== 'private') return; // सिर्फ पर्सनल चैट में काम करेगा

    if (!recoveryUsers[chatId]) recoveryUsers[chatId] = { state: 'IDLE' };
    let user = recoveryUsers[chatId];

    if (text === '/start') {
        bot.sendMessage(chatId, `🤖 <b>TRADE STAR LOSS RECOVERY BOT</b>\n\nभाई, आपका सारा लॉस रिकवर करवाने के लिए मैं तैयार हूँ।\n\nलॉस रिकवरी शुरू करने के लिए 👉 /recovery दबाएं।`, {parse_mode:'HTML'});
        user.state = 'IDLE';
    }
    else if (text === '/recovery') {
        bot.sendMessage(chatId, `😔 कोई बात नहीं भाई, लॉस होता रहता है।\n\n<b>आपका कुल लॉस (Loss) कितना है?</b> (सिर्फ अमाउंट लिखें, जैसे 600)`, {parse_mode:'HTML'});
        user.state = 'ASK_LOSS';
    }
    else if (user.state === 'ASK_LOSS' && !isNaN(text)) {
        ...
        user.target = Math.min(user.loss, user.wallet * 0.2); // टारगेट सेट
        
        let planMsg = `✅ <b>रिकवरी प्लान तैयार!</b>\n\n📉 <b>लॉस:</b> ₹${user.loss}\n💳 <b>वॉलेट:</b> ₹${user.wallet}\n🎯 <b>आज का टारगेट:</b> ₹${user.target} (लालच नहीं करना है)\n\n⚠️ <b>नियम:</b> आपको 6 लेवल का फंड मेंटेन करना है। अगर फंड कम है, तो नंबर पर दांव मत लगाना (Low Bal)।\n\nप्रेडिक्शन लेने के लिए 👉 /play दबाएं।`;
        bot.sendMessage(chatId, planMsg, {parse_mode:'HTML'});
        user.state = 'READY_TO_PLAY';
    }
    else if (text === '/play' && user.state === 'READY_TO_PLAY') {
        if (user.cooldown && Date.now() < user.cooldown) {
            bot.sendMessage(chatId, `🛑 <b>लालच नहीं भाई!</b>\n\nमैंने कहा था ना 1 घंटे बाद आना। अगर अभी खेलना है तो मेन चैनल पर जाओ 👉 ${CHANNEL_LINK}`, {parse_mode:'HTML'});
            return;
        }
        
        // बोट चेक करेगा कि मेन चैनल वाला प्रेडिक्शन (trend) कैसा है
...
bot.onText(/\/startsession/, (msg) => {
    ...
    isBotActive = true;
    bot.sendSticker(CHANNEL_CHAT_ID, STICKERS.predStart).catch(()=>{});
    bot.sendMessage(msg.chat.id, "✅ Session Started!");
});

bot.onText(/\/endsession/, (msg) => {
    if (msg.from.id.toString() !== ADMIN_ID) return;
    isBotActive = false;
    bot.sendSticker(CHANNEL_CHAT_ID, STICKERS.predEnd).catch(()=>{});
    bot.sendMessage(msg.chat.id, "✅ Session Ended!");
});

bot.onText(/\/skipmode/, (msg) => {
    if (msg.from.id.toString() !== ADMIN_ID) return;
    skipMode = !skipMode;
    bot.sendMessage(msg.chat.id, `⚙️ <b>Skip Mode is now ${skipMode ? "ON 🟢 (Will skip bad trends)" : "OFF 🔴 (Will give non-stop bets)"}</b>`, { parse_mode: 'HTML' });
});
...
bot.onText(/\/adminstats/, (msg) => {
    ...
    for(let pct in stats.pctTracker) {
        let d = stats.pctTracker[pct];
        let rate = Math.round((d.wins/d.total)*100);
        text += `👉 <b>${pct}% Range:</b> Win ${rate}% | Jackpots: ${d.jackpots}\n`;
    }
    bot.sendMessage(msg.chat.id, text || "No data yet.", { parse_mode: 'HTML' });
});

setInterval(monitorLoop, 5000);
console.log("🚀 Master AI V21 Booted Successfully...");
